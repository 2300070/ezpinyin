...
namespace EzPinyin.Spider
{
	...
	internal static class GuoxueSpider
	{
		...
		public static async Task LoadSampleAsync(WordInfo sample)
		{
			...
			try
			{
				...
				if (match.Success)
				{
					...
					{
						sample.GuoxuePinyin = Common.ParseWordPinyin(word, text.Substring(word.Length));
					}
				}
			}
			finally
		...
		public static async Task<CharacterInfo> LoadCharacterAsync(string character)
		{
			...
			string html = await Common.DownloadAsync($"http://www.guoxuedashi.net/zidian/so.php?sokeyzi={Uri.EscapeDataString(character)}&submit=&kz=1");
			if (html == null)
			{
				return null;
			}
			...
			if (html.TrimStart().StartsWith("<script"))
			{
				...
				if (index > 0)
				{
					...
					html = await Common.DownloadAsync($"http://www.guoxuedashi.net{url}");
					if (html == null)
					{
						return null;
					}
		...
		{
			string html = await Common.DownloadAsync($"http://www.guoxuedashi.net/hydcd/zi_{header}.html");
			if (html != null)
			{

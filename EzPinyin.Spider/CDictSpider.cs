...
namespace EzPinyin.Spider
{
	...
	internal static class CDictSpider
	{
		...
		public static async Task LoadSamplesAsync()
		{
			...
			await Common.ForEachAsync(arguments, async key =>
			{
				if (key[0] >= 'a' && key[0] <= 'z')
				{
		...
		public static async Task LoadSampleAsync(WordInfo sample)
		{
			...
			try
			{
				...
				if (match.Success)
				{
					...
					if (text == sample.Word)
					{
						sample.CDictPinyin = Common.ParseWordPinyin(text, match.Groups[2].Value);
					}
				}
			}
			finally
		...
		{
			if (key.IndexOfAny(Common.Dots) > -1)
			{
				return;
			}
			int page = 1;
			...
			do
			{
				string html = await Common.DownloadAsync($"https://search.cidianwang.com/?m=1&q={Uri.EscapeDataString(key)}&page={page}&y=0");
				if (string.IsNullOrEmpty(html))
				{
					return;
				}
				...
				if (match.Success)
				{
					...
					if (text == key)
					{
						...
						if (word.IsValid)
						{
							word.CDictPinyin = Common.ParseWordPinyin(text, match.Groups[2].Value);
						}
					}
				...
				if (matches.Count > 0)
				{
					for (int i = 0; i < matches.Count; i++)
					{
						...
						if (text.IndexOfAny(Common.Dots) > -1)
						{
							continue;
						}
						...
						if (word.IsValid)
						{
							word.CDictPinyin = Common.ParseWordPinyin(text, match.Groups[3].Value);
						}
					}
				}
				page++;
		...
		private static async Task LoadSamplesByLetterAsync(string letter)
		{
			...
			do
			{
				html = await Common.DownloadAsync($"https://www.cidianwang.com/cd/{file}");
				if (html != null)
				{
